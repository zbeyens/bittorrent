...
class Tracker:
    ...
    def get_file_info_2(self):
        ...
        for i in range(len(self.chunks)):
            ...
            for peer in peers:
                struct.pack_into(
                    '4B', *(int(x) for x in (self.ip_address_peers[peer][0]).split('.')))
                l += 4
                struct.pack_into(
                    '<Hxx', msg, l, self.ip_address_peers[peer][1])
                l += 4
        return msg
        ...
        while 1:
            ...
            if msg_header:
                ...
                if self.Packets.check_format(msg_version, msg_type) is False:
                    ...
                    self.Packets.send(
                        self, self.sock, self.version, self.type, self.length, self.tracker.msg_2)
                return False
            else:
